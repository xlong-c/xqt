...
def test_fp4_goal_differentiates_rationale_and_candidates() -> None:
    """DEBT-004: fp4/nvfp4 goals must not be silent no-ops."""

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    shape = GemmShape(m=128, n=128, k=4096)
    by_goal: dict[str, object] = {}
    for goal in ("latency", "throughput", "accuracy"):
        selection = select_gemm_engine(
            precision=PrecisionPolicy(mma="fp4"),
            shape=shape,
            device=device,
        ...
        assert selection.selected_engine == "tilelang"
        engines = tuple(c.engine for c in selection.candidates)
        text = " ".join(selection.rationale) + " " + " ".join(
            c.rationale for c in selection.candidates
        )
        by_goal[goal] = (engines, text)
        assert f"goal={goal}" in text or f"goal={goal}" in " ".join(
            c.rationale for c in selection.candidates
        ) or goal in text

    latency_engines, latency_text = by_goal["latency"]  # type: ignore[misc]
    throughput_engines, throughput_text = by_goal["throughput"]  # type: ignore[misc]
    accuracy_engines, accuracy_text = by_goal["accuracy"]  # type: ignore[misc]
    assert "torch" in accuracy_engines
    assert "torch" not in latency_engines
    assert "latency" in latency_text
    assert "throughput" in throughput_text
    assert "accuracy" in accuracy_text
    assert latency_text != accuracy_text

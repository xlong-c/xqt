...
def test_linear_marlin_int4_gpu_correctness_and_latency_smoke() -> None:
    ...
    weight = torch.randn(64, 64, device="cuda", dtype=torch.float16)
    qweight, scale = quantize_int4_weight(weight, group_size=64)
    kwargs = {
        "precision": "int4",
        "group_size": 64,
        "block_m": 32,
    ...
    for _ in range(3):
        linear_marlin_tilelang(x, qweight, scale, None, **kwargs)
    torch.cuda.synchronize()
    t0 = time.perf_counter()
    reps = 10
    for _ in range(reps):
        output = linear_marlin_tilelang(x, qweight, scale, None, **kwargs)
    torch.cuda.synchronize()
    elapsed_ms = (time.perf_counter() - t0) * 1000.0 / reps
    reference = linear_marlin_reference(
        x, qweight, scale, None, precision="int4", group_size=64
    )
    assert output.shape == reference.shape
    assert torch.allclose(output.float(), reference.float(), atol=2e-2, rtol=2e-2)

...
def test_prune_method_specs_cover_public_methods() -> None:
    assert SUPPORTED_PRUNE_METHODS == (
    ...
    specs = {item["method"]: item for item in supported_prune_method_specs()}
    assert set(specs) == set(SUPPORTED_PRUNE_METHODS)
    assert specs["global_l1_unstructured"]["baseline_kind"] == (
        "unstructured_sparsity_report"
    )
    assert specs["global_l1_unstructured"]["rewrites_structure"] is False
    assert specs["global_l1_unstructured"]["speedup_claimed"] is False
    assert specs["structured"]["rewrites_structure"] is True
    assert specs["nm_structured"]["pattern_present"] is True
    assert describe_prune_method("BLOCK_SPARSE").method == "block_sparse"


def test_prune_method_rejects_unknown_stage_method() -> None:
    try:
        build_stage_spec("prune", {"method": "l1"})
    except XQTConfigError as exc:
        assert "Unsupported prune method" in str(exc)
    else:
        raise AssertionError("expected unsupported prune method to fail")


def test_unstructured_prune_report_is_explicit_baseline() -> None:
    model = nn.Sequential(nn.Linear(4, 4), nn.Linear(4, 2))

    report = apply_global_l1_unstructured_pruning(model, 0.5).to_dict()
    report.update(prune_method_report_fields("global_l1_unstructured"))

    assert report["baseline_kind"] == "unstructured_sparsity_report"
    assert report["rewrites_structure"] is False
    assert report["speedup_claimed"] is False
    assert "does not imply latency speedup" in str(report["report_note"])


def test_sparse_prune_reports_include_pattern_and_speedup_status() -> None:
    model = nn.Sequential(nn.Linear(8, 4, bias=False), nn.Linear(4, 4, bias=False))

    nm_report = apply_nm_structured_sparsity(
        model,
        pattern_n=2,
        pattern_m=4,
    ).to_dict()
    block_report = apply_block_sparse_pruning(
        model,
        target_sparsity=0.5,
        block_shape=(2, 2),
    ).to_dict()

    assert nm_report["method_family"] == "semi_structured"
    assert nm_report["pattern_present"] is True
    assert nm_report["speedup_claimed"] is False
    assert block_report["method_family"] == "block_sparse"
    assert block_report["pattern_present"] is True
    assert block_report["speedup_verified"] is False
...
def test_structured_prune_support_matrix_covers_route_map() -> None:
    rows = structured_prune_support_matrix()
    by_granularity = {str(row["granularity"]): row for row in rows}
    ...
    for granularity in (
        ...
        assert granularity in by_granularity

    channel = by_granularity["channel"]
    assert channel["method"] == "structured"
    assert channel["rewrites_structure"] is True
    assert channel["exportable"] is True
    assert channel["speedup_verified"] is False

    attention = by_granularity["attention_head"]
    assert attention["canonical_granularity"] == "head"
    assert "Transformer" in attention["model_families"]

    token = by_granularity["token"]

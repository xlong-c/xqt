...
def test_assess_xqt_readiness_default_report_is_structured() -> None:
    ...
    assert report.status_counts.get("partial", 0) >= 3
    assert payload["status_counts"] == report.status_counts
    ...
    assert "cuda_device_capability" in fp4.checks
    assert "torch_cuda_available" in fp4.checks
    ...
    fp8 = scenarios["fp8_mma_hardware"]
    assert fp8.status in {"partial", "blocked"}
    assert "torch_scaled_mm_available" in fp8.checks
    assert "sm_meets_fp8_threshold" in fp8.checks
    assert "min_sm_for_fp8_mma" in fp8.checks
    assert fp8.checks["min_sm_for_fp8_mma"] == {"major": 8, "minor": 9}

...
def test_onnx_qdq_report_includes_calibration_and_quantized_ops(
    ...
    assert "runtime_diff" in report.metadata
    runtime_diff = report.metadata["runtime_diff"]
    assert isinstance(runtime_diff, dict)
    assert "runtime_diff_skip_reason" in runtime_diff
    assert runtime_diff["diff_ran"] is False

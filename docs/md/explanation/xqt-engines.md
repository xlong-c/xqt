...
| 词 | 含义 | 代码锚点 |
| --- | --- | --- |
| `engine` | XQT 内部 kernel / lowering 选择 | `OPERATOR_OPT_ENGINES`; `xqt/kernels/wrappers/capability.py` |
| `pattern` | engine registry 中的算子名 | 各 `*_KERNEL_REGISTRY` |
| `wrapper` | 模块级 adapter / candidate module / fallback 边界层 | `xqt/kernels/wrappers/`, `materialize.py` |
| `xqt.nn facade` | 语义块级 public `nn.Module` | `xqt/kernels/nn/` |
...
源码: `xqt/kernels/wrappers/capability.py` `_BASE_CAPABILITIES` + `describe_operator_engine_capability()`.
...
### 2.1 TileLang (`xqt/kernels/ops/_impl/engines/tilelang.py`)
...
kernel 实现目录: `xqt/kernels/ops/_impl/tilelang/` (`attention.py`, `conv.py`, `linear.py`, `linear_marlin.py`, `norm.py`, `gemm.py`, `int8_mma.py`, ...). `xqt/kernels/ops/_impl/tilelang/` 仅保留兼容 shim.

对应 wrapper / candidate module 目录: `xqt/kernels/wrappers/` (`attention.py`, `xqt_attention.py`, `linear.py`, `conv.py`, `conv3d.py`, `norm.py`, `dequant_gemm.py`, ...). 这些 wrapper 负责把 `nn.Module` 级语义翻译成 TileLang pattern 调用, 不属于 kernel 本体.

### 2.2 Triton (`xqt/kernels/ops/_impl/engines/triton.py`)
...
kernel 目录: `xqt/kernels/ops/_impl/triton/` (`gemm.py`, `linear.py`, `pointwise.py`, `mxfp_gemm.py`, ...). `xqt/kernels/ops/_impl/triton/` 仅保留兼容 shim.
...
### 2.3 CuTile (`xqt/kernels/ops/_impl/engines/cutile.py`)
...
capability resolve: `xqt.kernels.engine_resolve` (按 `required_capabilities`, 可选 preferred hint).
...
| --- | --- | --- |
| `xqt.nn facade` | `xqt/kernels/nn/` | 表达语义块身份和 runtime intent, 对外保持 `nn.Module` 语义 |
| `wrapper/materialize` | `xqt/kernels/wrappers/materialize.py`, `xqt/kernels/wrappers/` | 把 facade 或普通 module 翻译为 engine candidate, 处理 reshape/cache/fallback/metadata |
| `kernel` | `xqt/kernels/ops/_impl/`, `*_KERNEL_REGISTRY` | 只做 pattern 级 tensor 计算实现; 旧 `xqt/kernels/ops/_impl/` 仅保留兼容 shim |
...
| 入口 | 源码 | 当前行为 |
| --- | --- | --- |
| `xqt.convert(..., engine=...)` | `xqt/conversion.py`, `xqt/kernels/nn/conversion/` | 建 `OperatorContract`, 再经 `materialize_module(...)` 进入 wrapper / candidate 路径 |
| `xqt.nn.*` | `xqt/kernels/nn/` (`from xqt import nn`) | facade 构造带 engine intent; 导出 `Linear`, `Conv2d`, `LayerNorm`, `FeedForward`, `RMSNorm`, `Attention`, `TransformerBlock` |
| operator stage | `xqt/kernels/wrappers/execute.py`, `materialize.py` | workflow 内正式 plan + capability + fallback report |
...
```text
xqt/kernels/ops/_impl/
  tilelang/ triton/ cutile/ cutlass/ cute_dsl/  # 各 engine 实现与公共 pattern
xqt/kernels/wrappers/
  capability.py / execute.py / materialize.py / plan.py
  backends/              # engine adapter 与 registry mirror
...
xqt/compression/quant/capability.py  # quant backend + nature
xqt/compression/quant/quantizers/    # 算法实现
xqt/conversion.py        # convert facade
xqt/kernels/nn/          # semantic facade (`from xqt import nn`)
```

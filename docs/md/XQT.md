...
在本仓库内, XQT 是唯一推理优化主体. Python API 是主入口: `XQTOptimizationSession`, `xqt.convert(...)`, 当前真实的 `xqt.nn.FeedForward` / `RMSNorm` facade, 以及后续计划中的更多 semantic facade 用来表达模型替换, 算子 contract, runtime 状态和 benchmark/report. `triton`, `tilelang`, `cutlass`, `cute_dsl`, `cutile`, `custom_cuda` 是 XQT 内部 engine, 不是和 TensorRT / ONNX Runtime / OpenVINO 并列的外部 backend.

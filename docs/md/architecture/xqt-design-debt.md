...
## DEBT-004: gemm selector fp4/nvfp4 goal 感知尚未落地

**状态**: done  
**提出**: 2026-07-12  
**优先级**: medium  
**落地**: 2026-07-30 `select_gemm_engine` 对 `mma in {fp4,nvfp4}` 按 `goal=latency|throughput|accuracy` 写可区分 rationale; accuracy 额外暴露 torch 参考候选; selected 仍为 tilelang (唯一 executable 融合路径). 单测 `test_fp4_goal_differentiates_rationale_and_candidates`.

### 现象

`goal` 参数曾只做校验, 对 fp4/nvfp4 不改变候选说明, 调用方无法区分延迟/吞吐/精度意图.
...
- 不为 accuracy 把 auto selected_engine 改成 torch (避免性能路径静默降级)
- 不在 selector 内跑数值 diff

    ...
    *,
    ...
    if mma in {"fp16", "bf16"}:
        ...
        candidates.append(
            GemmEngineCandidate(
                engine="ptx_sm89",
                maturity="executable" if sm == 89 else "planned",
                dispatchable_by_gemm_with_precision=False,
                rationale=(
                ...
                caveats=(
                    "sm_89 only; batch=64 measured ~0.7x vs tilelang (loses "
                    "there), M>=256 single-layer can exceed tilelang "
                    "(~100 TOPS).",
                ),
            )
        )
        ...
        selected = "tilelang"
        rationale.append(f"{mma}: the TileLang packed dequant fused GEMM is the only real kernel.")
        candidates.append(
            GemmEngineCandidate(
                engine="tilelang",
                maturity="executable",
                dispatchable_by_gemm_with_precision=True,
                rationale=f"kernels/tilelang/gemm_builder.py fused dequant epilogue kernel for {mma}.",
            )
        )

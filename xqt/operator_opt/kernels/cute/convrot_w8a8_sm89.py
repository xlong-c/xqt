...
from xqt.operator_opt.kernels.cute.svdq_w4a4_sm89 import pack_scale
...
def _load_extension(dtype_key: str) -> Any:
    ...
    try:
        return load(
            ...
            sources=[
                str(_HERE / "convrot_w8a8_sm89_binding.cpp"),
                str(_HERE / "convrot_w8a8_sm89_kernel.cu"),
            ],
            extra_include_paths=[str(_NUNCHAKU_INCLUDE)],
            extra_cflags=["-O3", "-std=c++20", *dtype_flags],
            extra_cuda_cflags=[
                "-O3",
                "-std=c++20",

...
int scalar_kind(const torch::Tensor& tensor) {
    ...
    TORCH_CHECK(false, "native W4A4 small-N supports only float16 and bfloat16 tensors");
    return -1;
}
...
void quantize_weight(torch::Tensor input, torch::Tensor output, torch::Tensor scales) {
    ...
    TORCH_CHECK(n % kSmallBlockN == 0, "N must be padded to a multiple of 64");
...
void validate_gemm(
    ...
    TORCH_CHECK(
        padded_m % 256 == 0 && padded_n % kSmallBlockN == 0 && padded_k % 128 == 0,
        "invalid padded small-N GEMM shape");
    TORCH_CHECK(output.size(0) <= padded_m && padded_m - output.size(0) < 256, "invalid output M extent");
    TORCH_CHECK(
        output.size(1) <= padded_n && padded_n - output.size(1) < kSmallBlockN,
        "invalid output N extent");
    ...
    TORCH_CHECK(
        activation_scales.numel() == static_cast<int64_t>(padded_m) * padded_k / 64,
        "activation scale size mismatch");
    TORCH_CHECK(
        weight_scales.numel() == static_cast<int64_t>(padded_n) * padded_k / 64,
...
class BoundSVDQSmallNLinear {
public:
    BoundSVDQSmallNLinear(
        torch::Tensor act,
        torch::Tensor activation_scales,
        torch::Tensor lora_down,
        torch::Tensor lora_act,
    ...
    torch::Tensor run(torch::Tensor input) const {
        ...
        TORCH_CHECK(
            ...
            "input shape does not match the bound SVDQuant small-N runner");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
    ...
    module.def(
        ...
        [](torch::Tensor act,
           ...
           int64_t output_features,
           double lora_scale) {
            return std::make_unique<BoundSVDQSmallNLinear>(
                std::move(act),

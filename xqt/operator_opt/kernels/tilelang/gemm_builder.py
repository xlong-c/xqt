...
@lru_cache(maxsize=32)
def build_tilelang_nvfp4_unpack_dequant_kernel(
    n: int,
    input_features: int,
    group_size: int,
    *,
    ...
    if use_global_scale:
        ...
        @tilelang.jit(out_idx=[3], target=target)
        def unpack_dequant():
            @T.prim_func
            def main(
                packed_weight: T.Tensor(packed_shape, T.uint8),
...
@lru_cache(maxsize=32)
def build_tilelang_fp4_packed_activation_fused_gemm_kernel(
    m: int,
    n: int,
    input_features: int,
    group_size: int,
    *,
    ...
    if m % block_m != 0 or n % block_n != 0:
        raise ValueError(
            "minimal fused FP4 packed-activation TileLang GEMM requires m,n to be multiples of block sizes"
        )
    if activation not in {None, "gelu", "silu", "relu"}:
        raise ValueError(f"unsupported activation: {activation}")
    ...
    packed_k = (padded_input_features + 1) // 2
    groups = padded_input_features // group_size
    a_packed_shape = [m, packed_k]
    a_scale_shape = [m, groups]
    b_packed_shape = [n, packed_k]
    b_scale_shape = [n, groups, 1]
    bias_shape = [n]
    c_shape = [m, n]
    dtype = T.float16
    accum_dtype = T.float32
    ...
    def _decode_activation_nvfp4_e2m1(nibble):
        unsigned_nibble = nibble.astype(T.uint16)
        magnitude_code = T.bitwise_and(unsigned_nibble, 7)
        sign_bit = T.bitwise_and(unsigned_nibble, 8)
        magnitude = T.if_then_else(
    ...
    def _decode_weight_signed_int4(nibble):
    ...
    def fused_gemm_with_bias():
        @T.prim_func
        def main(
            packed_activation: T.Tensor(a_packed_shape, T.uint8),
            activation_scale: T.Tensor(a_scale_shape, dtype),
            packed_weight: T.Tensor(b_packed_shape, T.uint8),
            weight_scale: T.Tensor(b_scale_shape, dtype),
            bias: T.Tensor(bias_shape, dtype),
            out: T.Tensor(c_shape, dtype),
        ):
            with T.Kernel(
                ...
                for row_offset, feature in T.Parallel(block_m, input_features):
                    row = bx * block_m + row_offset
                    byte_u16 = packed_activation[row, feature // 2].astype(T.uint16)
                    low = T.bitwise_and(byte_u16, 15)
                    high = T.bitwise_and(T.shift_right(byte_u16, 4), 15)
                    nibble = T.if_then_else(feature % 2 == 0, low, high)
                    decoded_activation = _decode_activation_nvfp4_e2m1(nibble)
                    a_shared[row_offset, feature] = (
                        decoded_activation.astype(dtype)
                        * activation_scale[row, feature // group_size]
                    )
                for row_offset, feature in T.Parallel(block_n, input_features):
                    row = by * block_n + row_offset
                    byte_u16 = packed_weight[row, feature // 2].astype(T.uint16)
                    ...
                    high = T.bitwise_and(T.shift_right(byte_u16, 4), 15)
                    nibble = T.if_then_else(feature % 2 == 0, low, high)
                    decoded_weight = _decode_weight_signed_int4(nibble)
                    b_shared[row_offset, feature] = (
                        decoded_weight.astype(dtype)
                        * weight_scale[row, feature // group_size, 0]
                    )
                T.fill(acc_o, 0)
                T.gemm(
                    a_shared,
...
@lru_cache(maxsize=32)
def build_tilelang_nvfp4_packed_activation_fused_gemm_kernel(
    m: int,
    n: int,
    input_features: int,
    group_size: int,
    *,
    ...
    """Build a single-kernel NVFP4 activation/weight unpack, dequant, and GEMM path."""
    ...
    if m % block_m != 0 or n % block_n != 0:
        raise ValueError(
            "minimal fused NVFP4 packed-activation TileLang GEMM requires m,n to be multiples of block sizes"
        )
    if input_features % block_k != 0:
        raise ValueError(
            "minimal fused NVFP4 packed-activation TileLang GEMM requires input_features to be a multiple of block_k"
        )
    if block_k % group_size != 0:
        raise ValueError(
            "minimal fused NVFP4 packed-activation TileLang GEMM requires block_k to be divisible by group_size"
        )
    if activation not in {None, "gelu", "silu", "relu"}:
        raise ValueError(f"unsupported activation: {activation}")
    ...
    packed_k = (padded_input_features + 1) // 2
    groups = padded_input_features // group_size
    activation_packed_shape = [m, packed_k]
    activation_scale_shape = [m, groups]
    global_scale_shape = [1]
    if weight_is_prepacked:
        weight_packed_shape = [n // block_n, input_features // block_k, block_n, block_k // 2]
        weight_scale_shape = [
            n // block_n,
            input_features // block_k,
            block_n,
            block_k // group_size,
        ]
    else:
        weight_packed_shape = [n, packed_k]
        weight_scale_shape = [n, groups, 1]
    bias_shape = [n]
    output_shape = [m, n]
    dtype = T.float16
    accum_dtype = T.float32
    ...
    def fused_gemm_with_bias():
        @T.prim_func
        def main(
            packed_activation: T.Tensor(activation_packed_shape, T.uint8),
            activation_scale: T.Tensor(activation_scale_shape, dtype),
            activation_global_scale: T.Tensor(global_scale_shape, dtype),
            packed_weight: T.Tensor(weight_packed_shape, T.uint8),
            weight_scale: T.Tensor(weight_scale_shape, dtype),
            weight_global_scale: T.Tensor(global_scale_shape, dtype),
            bias: T.Tensor(bias_shape, dtype),
            ...
            with T.Kernel(
                ...
                for k_tile in T.Pipelined(
                    ...
                    for row_offset, feature_offset in T.Parallel(block_m, block_k):
                        ...
                        high = T.bitwise_and(T.shift_right(byte_u16, 4), 15)
                        nibble = T.if_then_else(feature % 2 == 0, low, high)
                        decoded_activation = _decode_nvfp4_e2m1(nibble)
                        a_shared[row_offset, feature_offset] = (
                            decoded_activation.astype(dtype)
                            * activation_scale[row, feature // group_size]
                            / activation_global_scale[0]
                        )
                    for row_offset, feature_offset in T.Parallel(block_n, block_k):
                        row = by * block_n + row_offset
                        feature = k_tile * block_k + feature_offset
                        if weight_is_prepacked:
                            byte_u16 = packed_weight[
                                by, k_tile, row_offset, feature_offset // 2
                            ].astype(T.uint16)
                            scale_value = weight_scale[
                                by, k_tile, row_offset, feature_offset // group_size
                            ]
                        else:
                            byte_u16 = packed_weight[row, feature // 2].astype(T.uint16)
                            scale_value = weight_scale[row, feature // group_size, 0]
                        low = T.bitwise_and(byte_u16, 15)
                        high = T.bitwise_and(T.shift_right(byte_u16, 4), 15)
                        nibble = T.if_then_else(feature % 2 == 0, low, high)
                        decoded_weight = _decode_nvfp4_e2m1(nibble)

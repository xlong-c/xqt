...
from ._common import _resolved_target_arch, _target_arch_mismatch_reason
...
def _feedforward_type() -> type[nn.Module]:
    ...
    from xqt.kernels.nn import FeedForward
...
def _build_triton_feedforward_candidate(
    ...
    if not supports_triton_feedforward(target_model):
        raise XQTBackendError(
            "Triton feedforward target requires an xqt.kernels.nn.FeedForward module"
        )

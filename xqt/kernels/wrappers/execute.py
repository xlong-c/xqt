...
from .patterns import scan_operator_candidate_reports
from .types import (
...
def execute_operator_optimization_plan(
    ...
    *,
    ...
    candidate_reports = scan_operator_candidate_reports(current_model, root_inputs)
    ...
    for target in plan.targets:
        ...
        compile_graph_report = torch_compile_graph_report(target, compile_explain)

        quant_guard_report = quant_runtime_guard_report(context, target)
        skip_reason = (
            str(quant_guard_report["reason"])
            if quant_guard_report["guard_applies"]
            else None
        )
        if (
            skip_reason is None
        ...
        fallback_detail = {
            ...
            "graph_break_count": compile_graph_report.get("graph_break_count"),
            "graph_breaks": list(compile_graph_report.get("break_reasons", [])),
            "compiled_regions": compile_graph_report.get("graph_count"),
            "explain": compile_graph_report,
        }
        skip_report = operator_planned_skip_report(
            target,
            capability.to_dict(),
            reason=fallback_reason,
        )
        ...
        if skip_reason is not None:
            reports.append(
                OperatorOptimizationReport(
                    ...
                    speedup=1.0,
                    numeric_diff=skipped_numeric_diff,
                    device=device,
                    dtype=dtype,
                    ...
                    metadata={
                        ...
                        "quant_runtime_guard": quant_guard_report,
                        "numeric_validation": operator_numeric_validation_report(
                            target,
                            status="not_run",
                            numeric_diff=skipped_numeric_diff,
                            reason=fallback_reason,
                        ),
                        "planned_skip_report": skip_report,
                        "graph_break_report": compile_graph_report,
                ...
                OperatorOptimizationReport(
                    ...
                    metadata={
                        ...
                        "fallback_detail": {
                            ...
                            "graph_break_count": compile_graph_report.get(
                                "graph_break_count"
                            ),
                            "graph_breaks": list(
                                compile_graph_report.get("break_reasons", [])
                            ),
                            "compiled_regions": compile_graph_report.get("graph_count"),
                            "explain": compile_graph_report,
                        },
        ...
        numeric_validation_status = "passed" if meets_numeric else "failed"
        numeric_validation_reason = None if meets_numeric else fallback_reason
        report_metadata: dict[str, Any] = {
            ...
            "numeric_validation": operator_numeric_validation_report(
                ...
                reason=numeric_validation_reason,
            ),
            "graph_break_report": compile_graph_report,
            "options": dict(target.options),
            "mode": target.mode,

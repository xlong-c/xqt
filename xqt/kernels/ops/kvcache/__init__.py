...
register_kernel(KernelSpec(op="kvcache.reshape_and_cache", backend=KernelBackend.TORCH, target="xqt.kernels.ops.kvcache:_reshape_and_cache_torch", format_signature=FormatSignature(in_place=True, description="reshape and cache")))
...
def reshape_and_cache(key, value, key_cache, value_cache, slot_mapping):
    ...
    return get_kernel("kvcache.reshape_and_cache", KernelBackend.TORCH)(key, value, key_cache, value_cache, slot_mapping)

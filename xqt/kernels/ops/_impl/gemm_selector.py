...
from xqt.kernels.ops.gemm.contracts import GemmProblem
...
class GemmEngineCandidate:
    ...
    ``xqt/kernels/wrappers/capability.py``: ``executable``, ``reference_guarded``,
...
def _sm_from_device(device: torch.device) -> int | None:
    ...
    ``advisor.py`` already imports ``xqt.kernels.ops._impl.gemm_precision``,
    so importing this selector back into ``advisor.py`` would form a circular
    import. This is a recorded, known

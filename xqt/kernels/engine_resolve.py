...
Canonical home is ``xqt.kernels.engine_resolve``. The registry table is
mirrored into ``xqt.kernels.registry.engine_registry`` at import time.
"""
...
__deprecated__ = "_ENGINE_REGISTRY is compatibility metadata; use xqt.kernels.registry.engine_registry"
...
from xqt.kernels.registry import mirror_engine_registration

for _registration in _ENGINE_REGISTRY.values():
    mirror_engine_registration(_registration)
...
def register_engine(registration: EngineRegistration) -> EngineRegistration:
    """Register an engine in both the compatibility and unified tables."""

    existing = _ENGINE_REGISTRY.get(registration.name)
    if existing is not None and existing != registration:
        raise ValueError(f"conflicting engine registration: {registration.name!r}")
    _ENGINE_REGISTRY[registration.name] = registration
    _ENGINE_PROVIDES[registration.name] = registration.provides
    mirror_engine_registration(registration)
    return registration

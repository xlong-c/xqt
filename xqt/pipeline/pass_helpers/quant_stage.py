...
def _maybe_run_graph_transforms(
    context: XQTContext,
    resolved_quant: QuantConfig | QuantStageSpec,
) -> list[dict[str, Any]]:
    """Run optional graph-level quant transforms before per-layer quantizers."""

    policy: dict[str, Any] = {}
    if isinstance(resolved_quant, QuantConfig):
        policy = dict(resolved_quant.policy or {})
    elif hasattr(resolved_quant, "policy"):
...
def _run_quant_with_resolved_config(
    ...
    *,
    ...
    if not enabled:
        return context
    transform_reports = _maybe_run_graph_transforms(context, resolved_quant)
    ...
    if transform_reports:
        quant_metrics["graph_transforms"] = transform_reports

...
def _resolve_component_strategy(
    strategy_value: Any,
    *,
    scheme: Any,
    policy: dict[str, Any],
    ...
    if strategy_value is None and isinstance(scheme, str):
        strategy_value = scheme
    if strategy_value is None:
        if scheme is not None:
            # A structured scheme is authoritative; the legacy strategy string
            # may be omitted entirely in that case.
            return None
        return require_supported_quant_strategy(
            None,
            location=location,
            policy=policy,
        )
...
def _merge_component_plan(
    ...
    if component_config is None:
        ...
        strategy = _resolve_component_strategy(
    ...
    scheme_input = (
        component_config.scheme
        if component_config.scheme is not None
        else quant_config.scheme
    )
    strategy = _resolve_component_strategy(
...
def _validate_quant_config_for_plan(quant_config: QuantConfig) -> None:
    ...
    has_selector = (
        ...
        or quant_config.scheme is not None

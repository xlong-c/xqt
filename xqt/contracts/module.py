"""Composite-precision GEMM payload contracts shared by quant and runtime.

Operator lowering types (``PrecisionPolicy``, ``ModuleContract``,
``FusionIntent``) live in ``xqt.kernels.precision``. This module keeps the
k-group composite GEMM schema so ``xqt.contracts`` does not import kernels.
"""
...
class CompositePrecisionGemmSpec:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "accumulation_dtype",
            _canonical_precision_name(self.accumulation_dtype),
        )
...
def get_module_contract(model: Any) -> dict[str, Any] | None:
    """Return a detached model module contract when one is attached."""

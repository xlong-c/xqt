...
from .methods import SUPPORTED_PRUNE_METHODS, describe_prune_method
...
def describe_prune_runtime_capability(
    *,
    ...
    method_spec = describe_prune_method(method)
    normalized_device = _normalize_device(device)

    if method == "global_l1_unstructured":
        return PruneRuntimeCapability(
            ...
            notes=(method_spec.report_note,),
            limitations=("Parameter masks do not change dense operator shapes.",),
            reason="Unstructured pruning is recorded as a sparsity baseline, not a verified acceleration path.",
            metadata={"device": normalized_device, "method_family": method_spec.family},
        )

    if method == "structured":
        return PruneRuntimeCapability(
            method=method,
            runtime="pytorch_rewrite",
            ...
            artifact_kind="pytorch_model",
            notes=(method_spec.report_note,),
            limitations=("Speedup must be established by a target-specific benchmark.",),
            reason="Structured pruning can rewrite supported PyTorch module topology.",
            metadata={"device": normalized_device, "method_family": method_spec.family},
        )
    ...
    raise ValueError(f"Unsupported prune capability method: {method}. Allowed: {allowed}")
...
def prune_runtime_capability_from_report(
    ...
    *,
    ...
    if method in {"global_l1_unstructured", "structured"}:
